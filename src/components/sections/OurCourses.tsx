"use client";

import { useEffect, useState } from "react";
import { Clock, BookOpen, ArrowRight } from "lucide-react";
import { homePage } from "@/lib/content";
import { Button } from "@/components/ui/Button";
import { useInView } from "@/hooks/useInView";
import { fadeUp } from "@/lib/animations";

export function OurCourses() {
  const { headline, subheadline, courses } = homePage.ourCourses;
  const { ref, isInView } = useInView<HTMLDivElement>({ threshold: 0.05 });
  const [entranceDone, setEntranceDone] = useState(false);

  useEffect(() => {
    if (isInView && !entranceDone) {
      const timer = setTimeout(() => setEntranceDone(true), 1200);
      return () => clearTimeout(timer);
    }
  }, [isInView, entranceDone]);

  return (
    <section className="bg-gray-50 py-20 md:py-24">
      <div ref={ref} className="mx-auto max-w-7xl px-6 md:px-10 lg:px-16">
        {/* Header */}
        <div
          className="mx-auto max-w-3xl text-center"
          style={fadeUp(isInView, 0)}
        >
          <h2 className="text-3xl font-bold text-navy md:text-4xl">
            {headline}
          </h2>
          <p className="mt-4 text-lg leading-relaxed text-gray-500">
            {subheadline}
          </p>
        </div>

        {/* Course cards */}
        <div className="mt-12 grid gap-6 md:grid-cols-3">
          {courses.map((course, i) => (
            <div
              key={course.title}
              className={`flex flex-col rounded-xl border bg-white p-6 ${
                entranceDone
                  ? "translate-y-0 border-gray-200 shadow-sm transition-all duration-300 ease-out hover:-translate-y-1 hover:border-blue-200 hover:shadow-lg"
                  : "border-gray-200 shadow-sm"
              }`}
              style={
                entranceDone ? undefined : fadeUp(isInView, 150 + i * 100)
              }
            >
              {/* Icon */}
              <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-blue-100">
                <BookOpen size={24} className="text-blue-500" />
              </div>

              {/* Title */}
              <h3 className="mt-4 text-xl font-bold text-navy">
                {course.title}
              </h3>

              {/* Description */}
              <p className="mt-2 flex-1 text-sm leading-relaxed text-gray-500">
                {course.description}
              </p>

              {/* Stats */}
              <div className="mt-4 flex items-center gap-4 border-t border-gray-100 pt-4">
                <div className="flex items-center gap-1.5 text-sm text-gray-500">
                  <Clock size={14} className="text-blue-500" />
                  <span className="font-semibold text-navy">
                    ~{course.hours}
                  </span>{" "}
                  hours
                </div>
                <div className="flex items-center gap-1.5 text-sm text-gray-500">
                  <BookOpen size={14} className="text-blue-500" />
                  <span className="font-semibold text-navy">
                    {course.chapters}
                  </span>{" "}
                  chapters
                </div>
              </div>

              {/* CTA */}
              <div className="mt-5">
                <Button variant="secondary" href={course.href}>
                  {course.cta} <ArrowRight size={14} className="ml-1 inline" />
                </Button>
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
