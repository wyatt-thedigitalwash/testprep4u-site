...
// TODO: Migrate to Supabase Storage with signed URLs for production.
// Currently writes to public/scorm/ which works for local dev but is
// read-only on Vercel's serverless environment.
...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Build the target path: scorm/{courseSlug}/part{N}/{sanitized-title}/
    ...
    // Find the SCORM entry file. Articulate Rise uses scormdriver/indexAPI.html
    ...
    if (!entryFile) {
      // If the zip had a single root folder, check one level deeper
      ...
      if (dirs.length === 1) {
        // SCORM package was wrapped in a folder
    ...
    // Update the module's scorm_entry_path in the database
